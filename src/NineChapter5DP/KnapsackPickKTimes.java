...
public class KnapsackPickKTimes {
	/*knapsackpickK T:O(n^3), Space:O(n)
	 * 	// Zero: null and negative check
	 * 	// First: DP dp[j] = Math.max(dp[j], dp[j- k * wt[i-1]] + k * val[i-1]), where 0<=k<=times[i-1]
		// NOTE: j--
		// NOTE: k has  times array
		// NOTE: nopick INSDIE THE LOOP => keepp updating dp[j]
	 * */
	public int knapsackpickK(int[] wt, int[] val, int[] times, int capacity){
		// Zero: null and negative check
